...
"######################################################################"
"#                                                                    #"
"#            Listmonk Installation and Hardening Script              #"
"#                                                                    #"
"#              Created by Honeytree Technologies, LLC                #"
"#                        www.honeytreetech.com                       #"
"#                                                                    #"
"#                    Listmonk: honeytree.social                      #"
...
validate_port() {
    local port=$1
    local excluded_ports=("80" "443" "9000")
...
while true; do
  ...
  if [ -n "$admin_password" ]; then
    ...
    echo "Admin password cannot be empty. Please enter admin password."
  fi
done
...
read -p "Enter the DB user name (Default: listmonk): " db_user
if [ -z "${db_user}" ]; then
  db_user=listmonk
fi
...
while true; do
  read -p "Enter a  new ssh port number (1-65535, excluding 80, 443, 9000): " port
  # Validate the input
  validate_port "$port"
  case $? in
    0)
...
services:
  ...
  app:
    ...
    command: ["sh", "-c", "sleep 20 && yes | ./listmonk --install --config config.toml && ./listmonk --config config.toml"]
    ports:
      - "9000:9000"
    networks:
      - listmonk
...
cat <<listmonk_env > ${work_dir}/config.toml
...
if nginx -v &>/dev/null; then
  ...
  rm /etc/nginx/sites-available/superdesk
  rm /etc/nginx/sites-enabled/superdesk
else
  sudo apt-get update
...
touch /etc/nginx/sites-available/superdesk

cat <<nginx_content >>/etc/nginx/sites-available/superdesk
server {
...
sudo ln -s /etc/nginx/sites-available/superdesk /etc/nginx/sites-enabled/
...
echo "Your database credentials are username: ${db_user}, password: ${db_password} and db name: listmonk"
echo "The Listmonk instance can be accessed on https://${domain_name}"
echo "Your admin credentials are username: ${admin_user} and password: ${admin_password}"
